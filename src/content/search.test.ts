...
describe("entity search", () => {
  it("matches IDs, localized names, slugs, case, and diacritics", () => {
    ...
    expect(filterEntities(entities, "PRODUCER.CHATEAU-LATOUR")[0]?.id)
      .toBe("producer.chateau-latour");
    expect(filterEntities(entities, "Chateau Latour")[0]?.id)
      .toBe("producer.chateau-latour");
    expect(filterEntities(entities, "cabernet-sauvignon")[0]?.id)
      .toBe("grape.cabernet-sauvignon");
  });

  it("combines text and entity-type filters", () => {
    const entities = getEntities();

    expect(filterEntities(entities, "bordeaux", "region").map((entity) => entity.id))
      .toEqual(["region.bordeaux"]);
    expect(filterEntities(entities, "bordeaux", "producer")).toEqual([]);
    expect(filterEntities(entities, "", "classification").map((entity) => entity.id))
      .toEqual(["classification.bordeaux-1855"]);
  });
